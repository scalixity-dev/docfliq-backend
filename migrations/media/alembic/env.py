...
# ── Path setup ────────────────────────────────────────────────────────────────
# env.py lives at: docfliq-backend/migrations/media/alembic/env.py
#   parents[0] = .../alembic/
#   parents[1] = .../media/
...
sys.path.insert(0, str(repo_root / "services" / "media"))  # makes `app` importable

# ── ALL model modules must be imported so Alembic's autogenerate detects them ─
from app.asset.models import MediaAsset  # noqa: E402, F401
...
url = os.environ.get("MEDIA_DATABASE_URL") or config.get_main_option(
    "sqlalchemy.url"
)
